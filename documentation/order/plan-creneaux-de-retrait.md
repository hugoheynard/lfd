...
| **Deux** importateurs de `pickupSlots` : le dialogue de retrait et la saisie de commande staff                                                        | `pickup-dialog.ts:18`, `acheminement-commande.ts:5`                               |
...
| `slot-step.ts` n'importe que le **type** ; `delivery-slots.ts` est une liste **écrite en dur** sans source                                            | `slot-step.ts:11`, `delivery-slots.ts:22-30`                                      |
...
- **Ne touche pas à la livraison.** ⚠️ Contrairement à ce qu'affirmait la
  première version, `delivery-slots.ts` **n'est pas** un lecteur de `opening` :
  c'est une liste écrite en dur, sans source (`vitruve`, mineure).
...
| Mineures : « cinq lecteurs » (il y en a deux), `delivery-slots.ts` écrit en dur, renvois de lignes faux, `Weekday` vient d'`address.ts`, `HoursForm` a deux consommateurs, le journal a deux classements | corrigées — §1, §5, D2, lot A et B                                                                                                                                    |
