...
> **Pas de résolution à la lecture.** On ne fait *jamais* « fiche de la déclinaison, sinon celle du
...
- **`variant_id` est à la fois PK et FK** (*shared primary key*). Pas d'`id` de substitution : un id
...
déjà codée : [`src/allergens`](../../../apps/chevallot-PIM-backend/src/allergens) (`toInco` /
`toGdsn`), détail dans `05-allergenes-gs1-inco.md` *(à porter)*. Codes GS1 **provisoires** — à
