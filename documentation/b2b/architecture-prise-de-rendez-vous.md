...
> **État au 2026-08-09 — R1 → R7 sont codés** : contrats, domaine pur,
> persistance, disponibilités staff, réservation client, file + page rendez-vous,
> reprise du `SupportRequest`. Écrit doc-first, ce document décrit désormais
> l'**existant**, à deux exceptions **explicites** : **R6 — notifications** (il
> n'y a aucun mailer dans le backend, cf.
> [`audit-flux-plateforme-admin.md`](audit-flux-plateforme-admin.md) §P1) et les
> décisions ouvertes du §7.
>
...
> Garde-fous d'architecture : [`todo-commercial-acquisition.md`](../todos/todo-commercial-acquisition.md).
...
C'est fait (R1 → R7). **Avant** : le client proposait une date et une demi-journée
dans le vide, le staff ne voyait qu'un booléen, et rien ne se clôturait (cf.
release §3.3). Ce paragraphe garde la trace du point de départ — le reste du
...
Trois blocs sur la sous-page existante ([`reglages/commercial`](../../apps/lfd-backoffice-frontend/src/app/reglages/commercial/reglages-commercial-page.ts)),
...
Le panneau existant ([`activation-support-panel`](../../apps/lfc-ecommerce-frontend/src/app/legacy/entreprises/activation-support-panel/activation-support-panel.ts))
...
- Le panneau contact générique ([`contact-panel.ts:38`](../../apps/lfc-ecommerce-frontend/src/app/legacy/contact/contact-panel/contact-panel.ts))
