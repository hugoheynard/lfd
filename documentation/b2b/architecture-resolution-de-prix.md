...
Reste **hors** de la frise à ce jour, et il faut le savoir en la lisant :
le **barème de volume**, qui a quitté `price_rules` pour sa propre table.
Une promotion qui compose avec un barème est un cumul réel, et la frise ne
le montre pas. `ladderAsRule` sait pourtant rendre l'échelle sous forme de
règle — le raccordement est une slice à part, pas un oubli.
