...
| **Saisonnalité / comparaison N-1**       | ❌ absent — _le manque n°1 pour ce métier_ |
...
10. **Cohortes de rétention inadaptées au B2B.** La cohorte est une **semaine
    ...
    « retenu » signifie _a commandé cette semaine-là_ : un client qui commande
...
12. **Deux sources de vérité pour le même fait.** La concentration est calculée
    **depuis le journal** (`activity_events`), le CA depuis la table `orders`. Or
    `ActivityRecorder` est _best-effort et n'échoue jamais_ : tout événement perdu
