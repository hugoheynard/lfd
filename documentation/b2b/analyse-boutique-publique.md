...
| Portes d'entrée : `/bienvenue` (trois champs, passkey) et `/ouverture-compte-pro` (déclare une société `pending`) ; l'index signale que `/bienvenue` **perd le profil en production** | `apps/lfc-ecommerce-frontend/src/app/app.routes.ts`, `documentation/README.md` |
