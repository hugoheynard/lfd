...
> qui compte pour publier sans casse — *à pousser*, *modifié en boutique*, *conflit*, le
...
> ne garde qu'un *hash* de la dernière poussée — assez pour « ne pas repousser l'identique »,
...
*dans l'admin Shopify* entre-temps. Un push écrase alors silencieusement cette édition. Et
...
Vocabulaire emprunté au merge à trois voies, parce que c'est *exactement* ça :
...
| **OURS** | le PIM **maintenant** (`projectProduct`) | calculé à la volée, pur | ce qu'on *voudrait* voir en ligne |
| **THEIRS** | Shopify **maintenant** (`inspect`/`listProducts`) | lu au vol, jamais canonique | ce qui est *réellement* en ligne |
...
futur** : quand la projection passera de *1 produit PIM → 1 produit Shopify* à *fiche×mode →
N produits Shopify* ([`projection-shopify.md`](./projection-shopify.md)), le modèle ne bouge
...
`unknown ≠ absent` : en dry-run ou boutique injoignable, `hRemote` est *inconnu*, pas *vide*.
...
- **Rollback n'efface rien** : il re-pousse un ancien payload, ce qui crée un *nouveau*
...
- Un snapshot porte de quoi rejouer *exactement* : `payload` (le `ShopifyProductPayload`),
...
- **Actions** : *Pré-push* (dry-run réel) · *Publier* · *Dépublier* · *Historique → Rollback*.
...
| **S5 — Dérive proactive** *(différé)* | webhook Shopify `products/update` → marque `remote_drift` sans poll | post-boucle |
...
1. **Détecter n'est pas résoudre.** Ce socle *montre* la dérive et le conflit ; il ne
