...
| `lint:journal-tracked`                          | tout handler de commande du PIM, des actes staff sur un compte, de `staff/` et — depuis le 2026-09-19 — de `b2b/order-waivers/` et `b2b/payments/` (client compris) journalise sous `UnitOfWork` — ou déclare `@sans-journal <raison>` / `@hors-transaction <raison>`, qui se greppent |
