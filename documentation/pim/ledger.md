...
**PM** — On fige comment publier vers Shopify **sans casse** : voir _ce qui partirait_ avant
...
n'efface pas v1). **Finding** : le dénominateur commun comparait le _titre de déclinaison_, que
...
seulement (le titre _produit_ reste comparé). Test de non-régression ajouté sur le cas exact.
...
`.run/`, `documentation/pim/`, et la base `lfc_pim` (utilisateur `lfc`). Volume Docker détruit et
...
_Vérifié après coup_ — base recréée, migrations appliquées, chaîne complète rejouée (famille →
...
_Vérifié en base_ — création complète relue : `VIEN-GALETT-ROIS`, marque, résumé, **2 visuels**
...
- **`nutrition_declaration`** — `variant_id` PK **et** FK (_shared primary key_), fiche
...
_Piège évité de justesse_ — `UnknownAllergenError` déclarait un champ `code`, qui **masquait** le
...
_Vérifié en vrai_ — catalogue UE (23 entrées) et Monde (24), création avec `["AW","AM","AE"]`
...
_Le secret ne va pas en base._ Réglages non sensibles en base et pilotables depuis l'écran ; **jeton
d'API dans l'environnement** (`SHOPIFY_ADMIN_TOKEN` via `AppConfig`). L'écran affiche sa _présence_,
...
_Le pilote réel n'est pas écrit — délibérément._ L'API Admin de Shopify est versionnée
...
_Ce qui a de la valeur, c'est la projection._ `projectProduct` est **pure** : catalogue → vocabulaire
...
_Frontières._ `CatalogueReader` devient le **seul** contrat exporté par le catalogue : l'adaptateur
ne voit ni ses dépôts, ni ses tables, ni ses commandes. Deux tables de binding, pas une : l'état de
synchro est au niveau **produit** (Shopify pousse produit + variantes ensemble), la _référence de
canal_ reste sur la **déclinaison** (R4) avec son index unique — l'unicité par canal, structurelle.
...
_Vérifié en vrai_ — réglages lus, enregistrés, push exécuté (2 produits « simulé »), **re-push
...
_Base_ — migration `socle_catalogue` : `category`, `product`, `product_variant`, plus un
...
_Collision de conception rencontrée en chemin_ — la déclinaison par défaut d'un produit sans option
...
_Frontières tenues_ — le domaine ne connaît ni Prisma ni HTTP. Les dépôts sont des classes
...
_Deux frictions des flags stricts, résolues sans concession_ — `exactOptionalPropertyTypes` impose
...
_Front_ — Angular zoneless, standalone, **signals uniquement** : zéro `FormsModule`, les champs sont
...
_Vérifié_ — tsc, lint, gate, **62 tests**, build AOT ; et l'API éprouvée au `curl` de bout en bout,
...
_Erreur de catégorie_ — `Sku = (variantUuid, channel, value)` avec `channel ∈ {shopify, caisse,
...
_Exigence « unicité par canal » satisfaite autrement, et mieux_ — un index unique sur la colonne de
...
_Le SKU n'est pas un module_ — pas d'entité, pas de service, pas de `POST /skus` (API en forme de
...
_Effet non évident de la normalisation_ — la valeur stockée étant toujours en majuscules, un index
...
_Unicité : trois couches, une seule garantie_ — value object (forme) · vérification en commande
...
_SKU par défaut_ — **signifiant** plutôt que séquentiel (il sera lu à voix haute au labo et cherché
...
_Deux relevés de vigilance_ — le partage front/back annoncé par la proposition était **impossible**
...
_Anti-drift_ — l'unité de décision est le **champ**, pas le catalogue : une matrice
...
_Promotions_ — elles ne sont **pas portables** : ce sont des programmes, pas des données. Shopify
...
_Pricing_ — correction d'un modèle mental : il n'y a **pas de prix canonique**. Le prix est une
...
_Construire vs acheter_ → [ADR-15](./adr.md#adr-15--construire-un-pim-minimal-plutôt-quen-acheter-un).
...
_Fond_ — le modèle était **conçu table-first** puis étiqueté « event-sourced » : aucune commande,
...
_ADR-11 rétrogradé_ — sa justification (« zéro `created_at`/`updated_at` ») était esthétique **et
...
_Sécurité de la donnée réglementaire_ — `PRODUCT ||--|| NUTRITION_INFO` (1:1 **obligatoire**) forçait
...
_Cohérence_ — `01-produit.md` re-spécifiait le socle **différemment** de `02` (`name` string vs
...
_Nouveau_ — [ADR-13](./adr.md#adr-13--composition-par-tables-satellites-canaux-au-bord) et
...
> session et ne prouve donc pas que la base est joignable. Le fail-fast ne couvre que la _config_.
...
essayé **et abandonné** : son preset impose un TS _project-references_ incompatible avec Angular
...
**PM** — Les décisions sont tracées avec leur _pourquoi_, et les questions encore ouvertes sont
visibles au lieu de rester dans une tête.

**Tech** — `documentation/pim/` : index, **ADR-01→12**, **todo** (décisions ouvertes D1–D6 +
...
**PM** — On sait maintenant précisément ce qu'_est_ un produit — et surtout ce qui n'en fait **pas**
...
> ⚠️ **Révisé le 2026-07-21** — décision rétrogradée en « event store _préparé, pas activé_ » après
