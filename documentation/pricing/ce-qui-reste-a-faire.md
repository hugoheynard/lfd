...
| **R26** | `Pricer` n'a **aucun** appelant — le plan de la porte est écrit              | 📐        | 4 lots, `vitruve`        |
...
| **R25**                                                                                                                     | 🟡 **`scope` est persisté depuis le 2026-09-09** — il ne l'avait jamais été, le schéma de relecture l'accueillant depuis le 09-03 avec un défaut qui couvrait le fil débranché. **Le scellement reste**, et sa forme est à reprendre : `sealedRuleIds` ne porte que le GAGNANT de l'étage scellé, en identifiants **nus** sans libellé — donc muets dès que la règle est supprimée. Et les causes d'une règle sans effet sont **cinq** (expirée, suspendue, hors audience, hors portée, sous le seuil), pas trois. Détail et septième forme envisagée : [journal](journal-de-remediation.md) §R25.7. `supersedes` attend **R27**                                                                                                                                                                                                                                                                                                                                                                                                        | Reconcevoir la forme avant toute migration. Une colonne pour un fait incomplet et illisible serait le pire usage d'un geste irréversible.                                                                                                                                                                                                                                                                                                     |
| **R27**                                                                                                                     | 🟠 **La trace de prix part au client.** `GET /orders/mine`, `GET /orders/:id` et `GET /companies/:id/orders` servent `OrderView` **sans rétrécissement**, donc `pricing.steps` avec le libellé commercial de chaque règle — alors que `POST /orders/quote` a été rétrécie exactement pour ça (« les rivales qu'elle a évincées, le plancher, c'est-à-dire la marge »). Trouvé le 2026-09-09 en bâtissant R25, qui aurait élargi le trou. **Aucun front ne lit `line.pricing`** : le rétrécissement ne casse rien                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Une `CustomerOrderView` et son mapper **champ par champ**, comme `toCustomerQuote` — jamais un `omit`. Débloque `supersedes` de R25.                                                                                                                                                                                                                                                                                                          |
...
| **R26**                                                                                                                     | 🔵 **`Pricer` n'a aucun appelant** — `app.module.ts:110` l'importe, personne ne l'injecte. Les trois consommateurs refont la chorégraphie, le mapping est copié six fois, `pricerFor` rend `null`, « ce qu'on écarte » a trois formes, `pricing → orders` pour le catalogue, le domaine importe `@lfd/contracts`, la lecture est hors bus                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | **Le plan est écrit** : [`plan-la-porte-du-prix.md`](plan-la-porte-du-prix.md), 2026-09-09. Il **corrige** C.4 sur deux points que le travail du 09 a périmés — `pricing → catalog` ferait désormais un cycle, et le septième consommateur a contourné la façade parce qu'elle relit un catalogue qu'il a déjà. La porte prend des **articles**, pas des SKU. Quatre lots ; le premier ne change aucun comportement. `vitruve` avant de bâtir |
