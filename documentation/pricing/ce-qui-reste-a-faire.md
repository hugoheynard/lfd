...
| ~~R25~~ | ~~La trace figée : le scellement, et ce que le moteur écarte~~                 | ✅      | **close le 2026-09-09**  |
...
**Quinze entrées sur vingt-huit sont closes** — R1, R3, R5, R7, R8 et R20 le
jour même de ce registre, puis R22, R23, **R17**, **R26**, **R18**, **R19**,
**R27** et **R25** dans la foulée.
**Douze ont été ajoutées le 2026-09-08** par le troisième regard (R15 à R26), et
**R15 a été ramenée de 🔴 à 🟠 le 2026-09-09**. Il reste **treize** entrées,
...
| ~~R25~~                                                                                                                     | ✅ **Close le 2026-09-09, en quatre lots.** Lots 1 et 2 : Le moteur consigne ce qu'il a **regardé sans l'appliquer** — évincée, scellée, sous le seuil, chacune avec son **libellé** —, `assembled()` rend ses recalés (un barème ou une mercuriale sans palier atteint n'atteignait jamais le moteur), et la colonne `pricing_rejected` porte les **trois** états. `sealedRuleIds` en est désormais une dérivation. Détail : [journal](journal-de-remediation.md) §R25 lots 1-2, et le [plan](plan-la-trace-qui-explique.md), publié avec ses trois âges. Lot 3 : `price-path` prend une `PriceChain`, `frozenChainOf` la fabrique depuis la trace figée, et le panneau **dit** qu'il est figé. Le bouton « pourquoi ce prix ? » n'existe qu'au comptoir. ✅ **Lot 4** : les causes non figées se relisent au tableau **daté**, et la période de pause au **journal** — la seule donnée qu'une reprise détruit. L'écran les marque « reconstruit », s'abstient sur une règle de segment, et n'affirme rien quand la relecture échoue. 🔴 **`supersedes` ne sera PAS persisté** : son défaut de relecture est `[]`, donc l'écrire rendrait à jamais indistinguables « aucune rivale » et « on ne consignait pas ». | —                                                                                                                                                                                                                                    |
...
| **R25 de ce registre** — la forme du scellement                    | ✅ Close le **2026-09-09**, en quatre lots, et **deux contradictions** : la première a démoli la colonne d'origine, la seconde la forme qui devait la remplacer. Ce qui reste debout est plus petit que ce qui était prévu — le fait tient dans une seule colonne, et trois causes sur quatre se relisent au lieu de se figer.                                          |
