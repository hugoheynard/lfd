 ...
 * qu'elle peut EXERCER. Ils s'ajoutent — le premier ne disparaît jamais ; un
 * crédit suspendu n'y figure pas.
 */
export function settlementSummary(company: CompanyView): string {
  const terms = settlesOnAccount(company) ? company.grantedTerms : [];
  return ['per_order' as const, ...terms].map(settlementLabel).join(' · ');
}
