...
async function publicView(): Promise<PublicDeliverySettingsView> {
  return jsonBody<PublicDeliverySettingsView>(
    await ctx.http().get("/delivery-settings").expect(200),
  );
}

async function adminView(): Promise<DeliverySettingsView> {
  return jsonBody<DeliverySettingsView>(await staff().get("/admin/delivery-settings").expect(200));
}
...
describe("le réglage de livraison", () => {
  it("ligne absente : ouvert aux deux, sans auteur ni instant — sur les deux routes", async () => {
    ...
    expect(await adminView()).toEqual(DEFAULT_DELIVERY_SETTINGS);
    expect(await ctx.prisma.deliverySettings.count()).toBe(0);
  });
  ...
  it("ferme une clientèle en laissant l'autre, et fige qui et quand", async () => {
    ...
    const row = await ctx.prisma.deliverySettings.findUniqueOrThrow({ where: { key: "delivery" } });
  ...
  it("ne sert JAMAIS l'auteur ni l'instant sur la route publique", async () => {
    ...
    const body = (await ctx.http().get("/delivery-settings").expect(200)).body as Record<
      string,
      unknown
    >;
  ...
  it("un second patch part de l'état posé, pas du défaut", async () => {
    ...
    expect(await ctx.prisma.deliverySettings.count()).toBe(1);
  });

  it("refuse un patch vide (400) — il faut dire quelle clientèle change", async () => {
    await staff().patch("/admin/delivery-settings").send({}).expect(400);
  });

  it("refuse l'écriture sans jeton staff", async () => {
    const response = await ctx.http().patch("/admin/delivery-settings").send({ openToB2c: false });
  ...
  it("journalise le geste avec l'état remplacé", async () => {
    ...
    const facts = await ctx.prisma.activityEvent.findMany({
      where: { type: "delivery_settings.updated" },
    });
