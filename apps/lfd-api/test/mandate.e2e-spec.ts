 ...
 * paiement**. Une frontière doublée : le verifier staff — le prestataire de
 * mandats (Stripe) a été supprimé le 2026-09-19, aucun mandat Stripe en
 * production (Hugo). Le stockage objet est réel (MinIO) — le mandat signé part
 * vraiment, sous une clé qui porte la société ET le mandat.
 */
 ...
 */
async function seedMandate(status: "active" | "draft" = "active"): Promise<string> {
...
describe("Mandat — le canal d'enregistrement est fermé", () => {
  ...
  it("rend le mandat semé, et ne laisse JAMAIS sortir de quoi débiter", async () => {
    ...
    expect(JSON.stringify(response.body)).not.toMatch(/paymentMethodId|stripeCustomerId/u);
  });
});

describe("Mandat — révocation puis remplacement", () => {
  it("date la révocation, et rouvre la voie", async () => {
    await seedMandate();
...
describe("Mandat — la preuve", () => {
  ...
  it("refuse d'activer un brouillon dont le scan n'est pas déposé", async () => {
    const mandateId = await seedMandate("draft");
