...
describe("SeededProductCatalog", () => {
  ...
  it("range CHAQUE SKU du seed — aucun préfixe inconnu", async () => {
    ...
    expect((await catalog.all()).length).toBeGreaterThan(0);
  });

  it("rend le catalogue dans l'ordre de la vitrine", async () => {
    const items = await catalog.all();
    const shelves = items.map((item) => CATALOG_CATEGORY_ORDER.indexOf(item.category));

    expect(shelves).toEqual([...shelves].sort((a, b) => a - b));
  });

  it("trie par nom à l'intérieur d'un rayon", async () => {
    const items = await catalog.all();
    const pains = items.filter((item) => item.category === "pain").map((item) => item.name);

    expect(pains).toEqual([...pains].sort((a, b) => a.localeCompare(b, "fr")));
  });

  it("`all` et `resolve` parlent des mêmes articles — même prix, même TVA", async () => {
    ...
    for (const item of await catalog.all()) {
      expect(await catalog.resolve(item.sku)).toEqual(item);
    }
  });

  it("ignore un SKU inconnu plutôt que d'en inventer un", async () => {
    expect(await catalog.resolve("ZZZ-999")).toBeNull();
  });
});
