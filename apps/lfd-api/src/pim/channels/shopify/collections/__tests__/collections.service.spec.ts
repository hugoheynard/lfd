...
import { DryRunShopifyCollectionsGateway, LiveShopifyCollectionsGateway } from "../gateway.js";
...
describe("ShopifyCollectionsService (dry-run)", () => {
  it("inspecte : présentes amorcées, tva-20 manquante, tva-8-5 orpheline", async () => {
    ...
    const missing = reconciliation.rows.filter((row) => !row.present).map((row) => row.handle);
  ...
  it("pousse : crée la manquante et referme la boucle", async () => {
    ...
    expect(result.reconciliation.rows.find((row) => row.handle === "tva-20")?.present).toBe(true);
  });
});
