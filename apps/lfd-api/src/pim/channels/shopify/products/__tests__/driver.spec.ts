...
const OK_RESPONSE = {
  productSet: {
    product: {
      ...
      handle: "croissant",
      variants: {
        nodes: [{ id: "gid://shopify/ProductVariant/9", sku: "PATI-CROISSANT" }],
      },
    },
    userErrors: [],
  },
...
class GraphqlRecorder {
  ...
  readonly graphql = (query: string, variables: Record<string, unknown> = {}): Promise<unknown> => {
    this.calls.push({ query, variables });
    return Promise.resolve(this.response);
  };
}

async function driverWith(recorder: GraphqlRecorder): Promise<LiveShopifyDriver> {
...
describe("LiveShopifyDriver", () => {
  it("renvoie le GID produit + la map SKU → GID variante", async () => {
    ...
    expect(result.variantGids["PATI-CROISSANT"]).toBe("gid://shopify/ProductVariant/9");
  });
  ...
  it("jette une ShopifyRejectedError sur userErrors", async () => {
    ...
    await expect(driver.push(PAYLOAD)).rejects.toBeInstanceOf(ShopifyRejectedError);
  });
});
