...
import { ScannedDocument, SCANNED_DOCUMENT_MAX_BYTES } from "../scanned-document.js";
...
describe("ScannedDocument", () => {
  it("accepte un PDF et dérive le content-type de son contenu", () => {
    const file = ScannedDocument.create("  extrait-kbis.pdf ", pdf());
  ...
  it("refuse un fichier qui n'est pas un PDF, même bien nommé", () => {
    ...
    expect(() => ScannedDocument.create("virus.pdf", Buffer.from("MZ\x90\x00", "latin1"))).toThrow(
      InvalidScannedDocumentError,
    );
  });

  it("refuse un fichier vide ou sans nom", () => {
    expect(() => ScannedDocument.create("kbis.pdf", Buffer.alloc(0))).toThrow(/vide/u);
    expect(() => ScannedDocument.create("   ", pdf())).toThrow(/nom de fichier/u);
  });

  it("refuse un fichier trop volumineux", () => {
    const tooBig = Buffer.concat([pdf(), Buffer.alloc(SCANNED_DOCUMENT_MAX_BYTES)]);
    expect(() => ScannedDocument.create("gros.pdf", tooBig)).toThrow(/taille maximale/u);
  });
});

describe("ScannedDocument — la photo comme secours", () => {
  it("accepte une photo JPEG et le dit dans son content-type", () => {
    ...
    const file = ScannedDocument.create("kbis.jpg", jpeg);
  ...
  it("accepte un PNG", () => {
    ...
    expect(ScannedDocument.create("kbis.png", png).contentType).toBe("image/png");
  });
  ...
  it("accepte un HEIC — ce que rend un iPhone sans conversion", () => {
    ...
    expect(ScannedDocument.create("IMG_0042.HEIC", heic).contentType).toBe("image/heic");
  });
  ...
  it("refuse toujours ce qui n'est ni PDF ni image, quel que soit le nom", () => {
    ...
    expect(() => ScannedDocument.create("kbis.jpg", Buffer.from("GIF89a", "latin1"))).toThrow(
      InvalidScannedDocumentError,
    );
  });
});
