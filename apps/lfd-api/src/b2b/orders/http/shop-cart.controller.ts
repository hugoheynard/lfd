...
/**
 * **Le panier en cours du client**, gardé chez nous — un par espace de travail.
 *
 ...
 * de rester visitable.
 *
 * ## L'espace vient de la porte
 *
 * Le panier servi est celui de la **société agissante** (`@ActingCompany()`),
 * résolue depuis l'en-tête d'espace confronté aux rattachements — `null` pour le
 * perso. Jamais d'un corps ni d'un paramètre : écrire le panier d'une maison à
 * laquelle on n'appartient pas n'a pas de syntaxe. Sans en-tête, l'espace suit
 * les règles de `resolveCompany` : ouvrir sa première société ou en rejoindre
 * une seconde change donc le panier servi.
 *
 ...
 * cette personne dans cet espace — la rejouer deux fois ne crée pas deux
 * paniers. C'est le même parti que le brouillon du back-office, dont ce
 * contrôleur est le frère.
 *
 ...
 * onglets ouverts sur le même compte et le même espace se recouvrent donc l'un
 * l'autre — ce que le `localStorage` faisait déjà, à ceci près qu'il ne le
 * faisait qu'entre onglets du même navigateur.
 */
...
export class ShopCartController {
  ...
  async mine(
    ...
    const cart = await this.queries.execute<GetShopCartQuery, ShopCartView | null>(
      new GetShopCartQuery(user.userId, companyId),
    );
    return { cart };
  }
  ...
  async save(
    ...
    return this.commands.execute<SaveShopCartCommand, ShopCartView>(
      new SaveShopCartCommand(user.userId, companyId, payload),
    );
  }
}
