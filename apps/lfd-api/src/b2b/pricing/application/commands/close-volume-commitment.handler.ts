...
> {
  ...
  async execute(command: CloseVolumeCommitmentCommand): Promise<void> {
    ...
    const { companyId } = commitment.asCommitment;
    const companyName = await this.companies.nameOf(companyId);
    await this.uow.run(async () => {
      await this.commitments.save(
        commitment.close(command.staffUserId, this.clock.now(), command.reason),
      );
      await this.events.publishTraced(
        new VolumeCommitmentClosedEvent(command.id, command.reason, companyId, companyName),
      );
    });
  }
}
