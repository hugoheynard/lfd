...
const at = (companyId: string, unitPriceMillicents: number, sku = "baguette"): NegotiatedPrice => ({
  sku,
  companyId,
  unitPriceMillicents,
});
...
describe("benchmarkByProduct", () => {
  ...
  it("moyenne les deux du milieu sur un nombre pair", () => {
    ...
    expect(entry?.medianMillicents).toBe(120);
  });
  ...
  it("encaisse un contrat exceptionnel bien mieux que la moyenne — c'est POURQUOI", () => {
    ...
    expect(entry?.medianMillicents).toBe(105);
    expect(entry?.medianMillicents ?? 0).toBeGreaterThan(mean);
  });
  ...
  it("compte UN client une fois, même avec trois paliers", () => {
    ...
    expect(entry?.lowMillicents).toBe(120);
  });
