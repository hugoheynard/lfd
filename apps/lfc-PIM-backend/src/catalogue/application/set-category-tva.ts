...
export class SetCategoryTvaHandler implements ICommandHandler<
  SetCategoryTvaCommand,
  void
> {
  constructor(
  ...
  async execute(command: SetCategoryTvaCommand): Promise<void> {
    ...
    await this.categories.setTva(
      command.id,
      command.emporterTvaId,
      command.surPlaceTvaId,
    );
  }
}
