...
function availabilityOf(taken: readonly string[]): SkuAvailability {
  ...
  return {
    isTaken: (candidate: Sku): Promise<boolean> =>
      Promise.resolve(set.has(candidate.value)),
  };
}
...
describe('génération de la référence par défaut', () => {
  ...
  describe('optionsDiscriminator', () => {
    ...
    it('garde un mot seul plus long, sinon il serait illisible', () => {
      expect(optionsDiscriminator(new Map([['parfum', 'chocolat']]))).toBe(
        'CHOC',
      );
    });
  ...
  describe('racines', () => {
    it('compose famille + produit', () => {
      expect(productSkuRoot('patisserie', 'Tarte aux fraises')).toBe(
        'PATI-TARTE-FRAISE',
      );
    });
