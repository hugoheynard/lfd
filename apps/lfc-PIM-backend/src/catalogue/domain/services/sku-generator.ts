 ...
 */
export function optionsDiscriminator(
    ...
    const compact = parsed
      .map((segment) => {
        ...
        return parsed.length === 1
          ? segment.slice(0, SINGLE_SEGMENT_LENGTH)
          : segment.slice(0, 1);
      })
      .join('');
...
export function productSkuRoot(
  categorySlug: string,
  productName: string,
): string {
...
export async function proposeSku(
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    ...
    const candidate = Sku.create(
      truncateTo(root, SKU_MAX_LENGTH - suffix.length) + suffix,
    );
