...
export class ShopifyReconciliationService {
  ...
  async detail(handle: string): Promise<ReconciliationDetailView> {
    ...
    return {
      ...
      theirs: t,
      oursVsBase:
        oComparable && bComparable
          ? diffComparable(bComparable, oComparable)
          : [],
  ...
  private row(
    ...
    const b = base.get(handle);
    const localAhead = this.localAhead(o, b);
    const remote = this.remoteVerdict(
      b,
      theirs.get(handle) ?? null,
      remoteKnown,
    );
    ...
    const diffCount =
      o && b
        ? diffComparable(
            comparableFromPayload(b.payload),
            comparableFromPayload(o.payload),
          ).length
        : 0;
    return {
      handle,
  ...
  private localAhead(
    o: OursEntry | undefined,
    b: BaseEntry | undefined,
  ): boolean {
    return (
      o !== undefined &&
      b !== undefined &&
      fingerprint(o.payload) !== b.fullHash
    );
  }
  ...
  private remoteVerdict(
    ...
    return comparableHash(t) ===
      comparableHash(comparableFromPayload(b.payload))
      ? 'aligned'
      : 'drift';
  }
