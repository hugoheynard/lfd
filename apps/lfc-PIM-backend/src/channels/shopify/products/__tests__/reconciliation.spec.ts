...
function payload(
  overrides: Partial<ShopifyProductPayload> = {},
...
function remote(
  overrides: Partial<ShopifyProductSnapshot> = {},
...
describe('comparable (dénominateur commun)', () => {
  ...
  it('un prix changé en boutique casse l’empreinte comparable', () => {
    ...
    expect(comparableHash(drifted)).not.toBe(
  ...
  it('un statut passé en DRAFT côté boutique casse l’empreinte', () => {
    ...
    ).not.toBe(comparableHash(comparableFromPayload(payload())));
  });
...
describe('diffComparable', () => {
  it('liste titre, statut et déclinaisons qui diffèrent', () => {
    ...
    const after = comparableFromPayload(
      payload({ title: 'Croissant beurre', status: 'DRAFT' }),
    );
