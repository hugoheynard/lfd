...
describe('projection Shopify', () => {
  ...
  describe('empreinte', () => {
    it('est stable pour un contenu identique', () => {
      expect(fingerprint(projectProduct(product()))).toBe(
        fingerprint(projectProduct(product())),
      );
    });
    ...
    it('ignore l’ordre des clés d’options', () => {
      ...
      expect(fingerprint(projectProduct(first))).toBe(
        fingerprint(projectProduct(second)),
      );
    });
    ...
    it('change dès que le contenu poussé change', () => {
      ...
      expect(fingerprint(projectProduct(product()))).not.toBe(
        fingerprint(projectProduct(renamed)),
      );
    });
  });
});
