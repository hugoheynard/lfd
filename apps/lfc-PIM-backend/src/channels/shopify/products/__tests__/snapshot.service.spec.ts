...
function fakePrisma(): {
  ...
  const table = {
    ...
    findUnique: ({
      ...
      where: { handle_version: { handle: string; version: number } };
    }) => {
...
const PAYLOAD: ShopifyProductPayload = {
  ...
  status: 'DRAFT',
  variants: [
    { sku: 'PATI-CROISSANT', title: 'Nature', options: {}, price: '1.30' },
  ],
};
...
function input(
  handle: string,
): Parameters<ShopifySnapshotService['record']>[0] {
  return {
    handle,
    productId: 'p1',
    hash: 'h',
...
describe('ShopifySnapshotService', () => {
  ...
  it('lève SnapshotNotFoundError sur une version absente', async () => {
    const { prisma } = fakePrisma();
    const snap = await build(prisma);
    await expect(snap.load('croissant', 99)).rejects.toBeInstanceOf(
      SnapshotNotFoundError,
    );
  });
});
