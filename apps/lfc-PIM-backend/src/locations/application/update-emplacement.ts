...
export class UpdateEmplacementHandler implements ICommandHandler<
...
> {
  ...
  async execute(command: UpdateEmplacementCommand): Promise<void> {
    ...
    const fields: EmplacementFields = {
      ...
      surPlace,
      baseUrl:
        patch.baseUrl !== undefined ? patch.baseUrl.trim() : current.baseUrl,
    };
    ...
    if (!surPlace) {
      ...
      await this.emplacements.replaceTables(
        id,
        syncTables(current.tables, patch.tableCount),
      );
    }
  }
}
