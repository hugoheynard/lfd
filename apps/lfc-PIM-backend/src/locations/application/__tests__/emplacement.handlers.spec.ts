...
} from '../delete-emplacement.js';
import {
  GenerateTableQrCommand,
  GenerateTableQrHandler,
} from '../generate-table-qr.js';
import {
  UpdateEmplacementCommand,
  UpdateEmplacementHandler,
} from '../update-emplacement.js';
...
class InMemoryEmplacements extends EmplacementRepository {
  ...
  setTableQr(
    ...
    this.mutate(id, (row) => ({
      ...
      tables: row.tables.map((t) =>
        t.number === tableNumber ? { ...t, qrCreated, token } : t,
      ),
    }));
    return Promise.resolve();
  }
  ...
  private mutate(
    id: string,
    map: (row: EmplacementRecord) => EmplacementRecord,
  ): void {
    const index = this.rows.findIndex((r) => r.id === id);
    if (index >= 0) {
...
describe('DeleteEmplacementHandler', () => {
  it('supprime l’emplacement', async () => {
    ...
    await new DeleteEmplacementHandler(repo).execute(
