...
export class GenerateTableQrHandler implements ICommandHandler<
...
> {
  ...
  async execute(command: GenerateTableQrCommand): Promise<string> {
    const emplacement = await requireEmplacement(
      this.emplacements,
      command.emplacementId,
    );
    requireTable(emplacement, command.tableNumber);
    const token = this.tokens.next();
    await this.emplacements.setTableQr(
      command.emplacementId,
      command.tableNumber,
      true,
      token,
