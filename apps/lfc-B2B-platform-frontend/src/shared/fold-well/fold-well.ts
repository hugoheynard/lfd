 ...
 * at all rather than an empty one. `titleVariant` picks its emphasis — `caps`
 * by default, `eyebrow` for a strip labelled inside something else. `[wellLead]` takes whatever identifies the
 ...
 *
 * `--lfc-well-gap` themes the space between items — on a rail it does not only
...
export class FoldWellComponent {
   ...
   *
   * `caps` by default: a well is a REGION, and a region announces itself. That
   * is the exact register the variant was added for — `eyebrow` whispers above
   * a thing, `caps` names the thing. The eyebrow grain stays available for a
   * strip labelled inside something else.
   */
  readonly titleVariant = input<'title' | 'eyebrow' | 'bar' | 'caps'>('caps');
