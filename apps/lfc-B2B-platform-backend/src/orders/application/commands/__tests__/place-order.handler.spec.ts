...
describe("PlaceOrderHandler", () => {
  it("refuse un non-membre par un 404 non-divulguant, sans rien écrire", async () => {
    ...
    const handler = new PlaceOrderHandler(guard(null, "active"), catalog, capturingRepo(sink));
  ...
  it("refuse une entreprise non activée (409), même à un membre", async () => {
    const sink = { placed: null as OrderToPlace | null };
    const handler = new PlaceOrderHandler(guard("member", "pending"), catalog, capturingRepo(sink));
  ...
  it("résout les prix au SERVEUR — le prix du client est ignoré", async () => {
    const sink = { placed: null as OrderToPlace | null };
    const handler = new PlaceOrderHandler(guard("member", "active"), catalog, capturingRepo(sink));
