...
const ALWAYS_ALLOWS: RateLimiter = { limit: () => Promise.resolve({ success: true }) };
const ALWAYS_REFUSES: RateLimiter = { limit: () => Promise.resolve({ success: false }) };
...
describe("trustedClientIp — l'identité ne vient QUE de Cloudflare", () => {
  it("lit cf-connecting-ip", () => {
    expect(trustedClientIp(requestWith({ "cf-connecting-ip": "9.9.9.9" }))).toBe("9.9.9.9");
  });

  it("ignore x-lfc-client-ip, même seul — c'est une valeur cliente", () => {
    expect(trustedClientIp(requestWith({ [CLIENT_IP_HEADER]: "1.2.3.4" }))).toBeNull();
  });

  it("ignore x-lfc-client-ip même quand cf-connecting-ip est présent", () => {
    const request = requestWith({ [CLIENT_IP_HEADER]: "1.2.3.4", "cf-connecting-ip": "9.9.9.9" });
...
describe("withTrustedClientIp — ce que le container reçoit", () => {
  it("écrase la valeur forgée par le client", () => {
    const request = requestWith({ [CLIENT_IP_HEADER]: "1.2.3.4", "cf-connecting-ip": "9.9.9.9" });
    expect(withTrustedClientIp(request).headers.get(CLIENT_IP_HEADER)).toBe("9.9.9.9");
  });
  ...
  it("le SUPPRIME faute d'IP de confiance, au lieu de laisser passer la valeur cliente", () => {
    ...
    expect(withTrustedClientIp(request).headers.get(CLIENT_IP_HEADER)).toBeNull();
  });
...
describe("guardedFetch — le câblage, pas seulement le helper", () => {
  ...
  it("clé le limiteur sur l'IP Cloudflare, pas sur l'en-tête client", async () => {
    ...
    await guardedFetch(request, limiter, () => Promise.resolve(new Response("ok")));
  ...
  it("ne limite pas un appel interne (pas d'IP), mais nettoie quand même l'en-tête", async () => {
    ...
    await guardedFetch(requestWith({ [CLIENT_IP_HEADER]: "1.2.3.4" }), limiter, (forwarded) => {
      seen.push(forwarded);
      return Promise.resolve(new Response("ok"));
    });
