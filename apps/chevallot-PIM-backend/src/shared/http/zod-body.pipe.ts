...
/**
 * Validation de la charge utile à la **frontière**.
 *
 * Elle ne remplace pas les invariants du domaine — les value objects se protègent
 * eux-mêmes, quel que soit le chemin d'entrée. Elle rend juste l'erreur lisible avant
 * qu'une requête manifestement mal formée n'atteigne un handler.
 */
    ...
    if (!result.success) {
      throw new InvalidPayloadError(
        result.error.issues.map(
          (issue) => `${issue.path.join('.') || 'corps'} : ${issue.message}`,
        ),
      );
    }
