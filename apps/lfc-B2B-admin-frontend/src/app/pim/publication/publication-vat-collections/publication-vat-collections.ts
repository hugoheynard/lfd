...
interface VatCollectionsView {
  readonly rows: readonly VatCollectionRow[];
 ...
 */
@Component({
  selector: 'app-publication-vat-collections',
  ...
  templateUrl: './publication-vat-collections.html',
  styleUrl: './publication-vat-collections.scss',
})
export class PublicationVatCollections {
  private readonly api = inject(ShopifyChannelApi);
  private readonly isBrowser = isPlatformBrowser(inject(PLATFORM_ID));

  protected readonly recon = signal<VatCollectionsView | null>(null);
  ...
  protected async inspect(): Promise<void> {
    ...
    this.error.set(null);
    try {
      const result = await this.api.inspectVatCollections();
  ...
  protected async pushMissing(): Promise<void> {
    ...
    this.error.set(null);
    try {
      const result = await this.api.pushVatCollections();
      this.mode.set(result.mode);
      this.recon.set(toView(result.reconciliation));
      this.lastInspectedAt.set(nowLabel());
    } catch {
...
function toView(reconciliation: Reconciliation): VatCollectionsView {
  const rows = reconciliation.rows.map<VatCollectionRow>((row) => ({
    handle: row.handle,
    title: row.title,
