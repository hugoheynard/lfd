...
export class TvaHttpApi {
  ...
  async list(): Promise<TvaRate[]> {
    const rows = await firstValueFrom(this.http.get<TvaRateView[]>(this.url('')));
    return rows.map((row) => ({ ...row }));
  }

  create(input: TvaRateInput): Promise<{ id: string }> {
  ...
  private body(input: TvaRateInput): Record<string, unknown> {
    return {
      name: input.name,
