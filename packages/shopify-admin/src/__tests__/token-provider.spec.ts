...
class FetchStub {
  ...
  readonly fetch = (
    ...
    if (next === undefined) {
      return Promise.reject(new Error("Aucune réponse en file pour cet appel."));
    }
...
describe("ShopifyTokenProvider", () => {
  ...
  it("rend le jeton statique legacy tel quel, sans aucun échange", async () => {
    const provider = new ShopifyTokenProvider(source({ token: "shpat_legacy" }));
  ...
  it("refuse sans aucun identifiant approvisionné", async () => {
    ...
    await expect(provider.accessTokenFor(DOMAIN)).rejects.toBeInstanceOf(ShopifyNotConfiguredError);
    expect(stub.calls).toHaveLength(0);
  });
  ...
  it("remonte une erreur transport quand le grant est refusé", async () => {
    ...
    await expect(provider.accessTokenFor(DOMAIN)).rejects.toBeInstanceOf(ShopifyTransportError);
  });
