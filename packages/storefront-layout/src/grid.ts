 ...
 *
 * Fonctions PURES : aucune dépendance, aucun effet. C'est ici que vit la règle
 * « deux objets ne se chevauchent jamais, aucun ne déborde des 5 colonnes ni
 * des R rangées » (`documentation/order/boutique-rayon-layout.md`, « Composer
 * une page »), et elle a TROIS lecteurs : l'éditeur la refuse à la pose, le
 * serveur à l'écriture (`b2b/storefront`, agrégat `Storefront`), la boutique
 * en compose ses pages (`documentation/order/plan-vitrine-enregistrement.md`,
 * D8). Une seule table des formes : deux implémentations divergeraient à la
 * première forme ajoutée.
 *
 * L'image vit dans `media.ts`, le défilement dans `carousel.ts`, la pile dans
 * `mobile.ts`.
 *
 * Positions 1-indexées, tailles en colonnes × rangées (cas de bureau).
 */

import type { CarouselSettings, ContentsMode } from "./carousel.js";
import type { MediaFit, MediaSide } from "./media.js";
 ...
 * avoir un produit en 2×2 »). Le contenu — produit ou info — s'associe sur
 * n'importe quelle forme ; la forme n'en présume rien.
 *
...
export interface PlacedBlock extends Cell {
  ...
  /** Absent = le défaut de la forme. Toujours parmi les côtés permis de sa forme (`media.ts`). */
